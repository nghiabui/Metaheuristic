...
public:
	...
	virtual unsigned int insertEdge(const unsigned int from,
	...
	virtual unsigned int insertLoop(const unsigned int from,
			const unsigned int to, const edaStopCriteria &con) {
...
protected:
	...
	virtual vector<unsigned int> findReadyTask() const {
		...
		for (intIter = taskList.begin(); intIter != taskList.end(); intIter++) {
			map<unsigned int, unsigned int>::const_iterator mci =
					taskStatus.find(*intIter);
	...
	virtual unsigned int checkTaskStatus(unsigned int taskID) {
		if (taskStatus[taskID] != STATUS_PENDING
				&& taskStatus[taskID] != STATUS_LOOPING) {
		...
		for (parentIter = parentTasks.begin(); parentIter != parentTasks.end();
				parentIter++) {
	...
	edaPopulation* chooseSolution(unsigned int taskID,
			edaPopulation &pop) {
		vector<unsigned int> parentTasks;
		parentTasks = taskDAG.getParentNodes(taskID);
		edaPopulation *Pop = NULL;
		if (taskStatus[taskID] == STATUS_LOOP_READY) {
			printf(" taskStatus[taskID] == STATUS_LOOP_READY  \n ");
			...
			vector<unsigned int>::iterator parentIter;
			for (parentIter = parentTasks.begin();
					parentIter != parentTasks.end(); parentIter++) {
	...
	bool allDone() {
		...
		do {
			...
			for (intIter = taskList.begin(); intIter != taskList.end();
					intIter++) {
...
//		printf("end of loop at info: %s:%d: \n", __FILE__, __LINE__);
		map<unsigned int, unsigned int>::const_iterator loopIter;
		for (loopIter = loopStatus.begin(); loopIter != loopStatus.end();
				loopIter++) {
